...
class Differ:
  ...
  def get_description(self):
    description = (
      "Welcome to DIFF LAB!\n\nTo start diffing, please enter the full names "
      "of the files you would like to compare below (including the extensions)"
  ...
  def compare_files(self, file_1_path, file_2_path):
    ...
    if df_1_col_len != df_2_col_len:
        self.show_error(f"Cannot compare files with different structure. File 1 has {df_1_col_len} columns and file 2 has {df_2_col_len} columns.")
        return
    ...
    if df_1_row_len != df_2_row_len:
        self.show_error(f"Cannot compare files with different structure. File 1 has {df_1_row_len} rows and file 2 has {df_2_row_len} rows.")
        return
    ...
    if df_1.equals(df_2):
      self.show_diff_complete_info(
        "The two files are identical to each other."
      )
      return
    ...
    self.show_diff_complete_info(
      "The two files are not identical to each other. Please open the file titled 'DIFF LAB OUTPUT' with the appropriate time stamp in your current directory to view the differences."
    )
